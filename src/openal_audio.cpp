...
int OpenALAudio::play_loop_ogg(const char *file_name, int repeat_offset, const DsVolume &vol) {
	int id;
	StreamContext *sc;
	...
	// std::cout << "play_loop_ogg: " << file_name << "\n";
	id = this->play_long_ogg(file_name, vol);
	if (id == 0)
		return 0;
	
	// std::cout << "play_long_ogg: " << file_name << " DONE \n";	
	sc = this->streams[id];
...
int OpenALAudio::play_long_ogg(const char *file, const DsVolume &vol) {
	...
	// std::cout << "starting play: " << file << "\n";
	
	os = new OggStream;
	if (!os->open(file)) {
		std::cout << "openal_audio.cpp: opening " << file << " failed\n";
		goto err;
	}
